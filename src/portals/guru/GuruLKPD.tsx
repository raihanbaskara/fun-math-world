...
import { FileText, Image as ImageIcon, Bot, ExternalLink, Plus, Trash2, Pencil, ChevronLeft, ChevronRight, CheckCircle2, Clock, Loader2, Sparkles, RefreshCw, BookOpen, Download, Trophy } from 'lucide-react';
          ...
          {(() => {
            ...
            return (
              <form onSubmit={handleSaveGrade} className="space-y-6 max-h-[75vh] overflow-y-auto pr-1">
                ...
                {/* Teacher Grading Dock (Double-Bezel Architecture) */}
                <div className="p-1.5 rounded-[2rem] bg-slate-900/[0.04] dark:bg-white/[0.05] ring-1 ring-slate-900/10 dark:ring-white/10 shadow-sm mt-4">
                  <div className="bg-white dark:bg-slate-900 rounded-[calc(2rem-0.375rem)] p-5 sm:p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-3.5 rounded-2xl border-2 border-slate-900/20 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-950 dark:text-slate-100 text-2xl font-black font-mono focus:bg-white focus:border-slate-950 dark:focus:border-slate-400 outline-none transition"
                        />
                      </div>
                      ...
                      <div className="sm:col-span-2 space-y-2">
                        ...
                        <textarea
                          ...
                          placeholder="Tulis catatan apresiasi atau arahan perbaikan konsep pecahan..."
                          className="w-full p-3 rounded-2xl border-2 border-slate-900/20 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-950 dark:text-slate-100 text-xs sm:text-sm font-medium focus:bg-white focus:border-slate-950 dark:focus:border-slate-400 outline-none leading-relaxed transition"
                        />

                        {/* Quick preset chips */}
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {[
                            "Pengerjaan sangat runtut & teliti",
                            "Konsep pecahan dan KPK sudah tepat",
                            "Perhatikan penyederhanaan bentuk akhir",
                            "Sudah bagus, tingkatkan ketelitian hitung"
                          ].map(preset => (
                            <button
                              key={preset}
                              type="button"
                              onClick={() => {
                                soundService.click();
                                setGradeFeedback(prev => prev ? `${prev} · ${preset}` : preset);
                              }}
                              className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-amber-100 dark:hover:bg-amber-950/40 text-[11px] font-mono font-bold text-slate-700 dark:text-slate-300 hover:text-amber-900 transition cursor-pointer border border-slate-200 dark:border-slate-700"
                            >
                              + {preset}
                            </button>
                          ))}
                    ...
                    <div className="flex justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                      <button
                        type="button"
                        className="px-5 py-2.5 rounded-xl font-mono font-bold text-xs bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition cursor-pointer"
                      ...
                      <button
                        type="submit"
                        className="px-6 py-2.5 rounded-xl font-mono font-black text-xs uppercase bg-[#ffe600] text-slate-950 border-2 border-slate-950 shadow-[3px_3px_0px_0px_#0f172a] hover:bg-yellow-400 cursor-pointer active:translate-x-0.5 active:translate-y-0.5 transition-all"
                      >
