...
export const GuruDaftarSiswa: React.FC<{
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto font-sans pb-12">
      ...
      <div className="relative rounded-3xl bg-[#a3e635] border-4 border-slate-950 dark:border-slate-800 p-6 sm:p-8 shadow-[8px_8px_0px_0px_#0f172a] dark:shadow-[8px_8px_0px_0px_#000000] overflow-hidden text-slate-950">
      ...
      <div className="rounded-3xl bg-white dark:bg-[#111827] border-4 border-slate-950 dark:border-slate-700 p-6 shadow-[7px_7px_0px_0px_#0f172a] dark:shadow-[7px_7px_0px_0px_#000000] space-y-6">
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          
          {/* Search Box */}
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-900 dark:text-slate-300" size={18} />
            <input
              type="text"
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              placeholder="Cari nama siswa atau NIS/Username..."
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-950 dark:border-slate-700 rounded-2xl text-xs sm:text-sm font-bold text-slate-950 dark:text-slate-100 shadow-[2px_2px_0px_0px_#0f172a] dark:shadow-[2px_2px_0px_0px_#000] outline-none focus:bg-white dark:focus:bg-slate-750"
            />
          </div>

          {/* Class Filter */}
          <div className="flex items-center gap-2">
            <span className="text-xs font-black text-slate-900 dark:text-slate-300 font-mono">Filter Kelas:</span>
            <select
              value={filterClass}
              onChange={e => setFilterClass(e.target.value)}
              className="p-2.5 bg-white dark:bg-slate-800 text-slate-950 dark:text-slate-100 text-xs font-black rounded-2xl border-2 border-slate-950 dark:border-slate-700 shadow-[2px_2px_0px_0px_#0f172a] dark:shadow-[2px_2px_0px_0px_#000] outline-none"
            >
                ...
                return (
                  <tr key={s.id} className="hover:bg-amber-50/50 dark:hover:bg-slate-800/60 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <img
                          src={s.avatar}
                          alt={s.name}
                          className="w-10 h-10 rounded-2xl border-2 border-slate-950 dark:border-slate-700 shadow-[2px_2px_0px_0px_#0f172a] object-cover bg-amber-100 shrink-0"
                        />
                        <div>
                          <div className="font-black text-slate-950 dark:text-slate-100 text-sm">{s.name}</div>
                    ...
                    <td className="p-4 text-center">
                      {hasLKPD ? (
                        <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-lime-100 text-emerald-950 border border-slate-950 text-xs font-black shadow-[1.5px_1.5px_0px_0px_#0f172a]">
                          <CheckCircle2 size={13} className="text-emerald-700" />
                          <span>Selesai ({lkpdSub.teacherScore || lkpdSub.aiScore || 90})</span>
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-300 dark:border-slate-700 text-xs font-bold">
                          <Clock size={13} />
                          <span>Belum</span>
                        </div>
                      )}
                    ...
                    <td className="p-4 text-center">
                      {hasLatsol ? (
                        <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-purple-100 text-purple-950 border border-slate-950 text-xs font-black shadow-[1.5px_1.5px_0px_0px_#0f172a]">
                    ...
                    <td className="p-4 text-center">
                      {hasEval ? (
                        <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-amber-100 text-amber-950 border border-slate-950 text-xs font-black shadow-[1.5px_1.5px_0px_0px_#0f172a]">
                    ...
                    <td className="p-4 text-center">
                      <div className="space-y-1 inline-block text-left w-24">
                        <div className="flex justify-between text-[10px] font-mono font-black text-slate-700 dark:text-slate-300">
                          <span>Progress</span>
                          <span>{avgProgress}%</span>
                        </div>
                        <div className="w-full h-2.5 bg-slate-200 dark:bg-slate-800 rounded-full border border-slate-950 dark:border-slate-700 overflow-hidden">
                          <div
